...
func (db *SqlDB) InsertExpression(ctx context.Context, expression *models.Expression, userID int) (int64, error) {
	...
	result, err := db.Store.ExecContext(ctx, q, expression.Text, userID, expression.Status, expression.Result)
	if err != nil {
		return 0, err
	}
	id, err := result.LastInsertId()
...
func (db *SqlDB) SelectExpressions(ctx context.Context, userID int) ([]byte, error) {
	...
	for rows.Next() {
		e := models.Expression{}
		err := rows.Scan(&e.ID, &e.Text, &e.Status, &e.Result)
		if err != nil {
			return nil, err
		}
...
func (db *SqlDB) SelectExprByID(ctx context.Context, id, userID int) (models.Expression, error) {
	...
	err := db.Store.QueryRowContext(ctx, q, id, userID).Scan(&e.ID, &e.Text, &e.Status, &e.Result)
	if err != nil {
		return e, err
	}
